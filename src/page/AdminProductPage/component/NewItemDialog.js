                ...
                <Col sm={4}>
                  ...
                  >
                    ...
                    {SIZE.map((item, index) => (
                      <option 
                      //inValid={true} 
                              value={item.toLowerCase()} // 선택하면 formData에 저장될 값
                              disabled={stock.some( // 이미 선택된 사이즈면 비활성화
                              (size) => size[0] === item.toLowerCase()
                              )}
                              key={index}// React에서 리스트 렌더링 시 필수
                              > {item}
